...
(c.val("Wrong Code"),c.prop("type","text"),$("#"+b.attr("id")).addClass("ui-focus"),setTimeout(function(){$("#"+b.attr("id")).removeClass("ui-focus")},400)):"-"==e?(c.val(""),c.prop("type","password")):("Wrong Code"==c.val()&&(c.val(""),c.prop("type","password")),c.val(c.val()+e))}});
...
$(document).delegate('div[data-widget="plot.minmaxavg"]',{update:function(d,a){for(var b=$(this).attr("data-axis").explode(),c=$(this).attr("data-unit"),e=a[0],f=a[1],g=[],h=0;h<e.length;h++)g[h]=[e[h][0],e[h][1],f[h][1]];$("#"+this.id).highcharts({chart:{type:"columnrange",inverted:!1},series:[{data:g,enableMouseTracking:!1},{type:"line",data:a[2]}],xAxis:{type:"datetime",title:{text:b[0]}},yAxis:{min:$(this).attr("data-ymin"),max:$(this).attr("data-ymax"),title:{text:b[1]}},plotOptions:{columnrange:{dataLabels:{enabled:!0,
formatter:function(){return""!=c?parseFloat(this.y).transUnit(c):parseFloat(this.y).transFloat()}}}},tooltip:{pointFormatter:function(){var a=this.y,a=""!=c?parseFloat(this.y).transUnit(c):parseFloat(this.y).transFloat();return'<span style="color:'+this.color+'">\u00d8</span>  <b>'+a+"</b><br/>"}},legend:{enabled:!1}})},point:function(d,a){var b=$(this).attr("data-count");1>b&&(b=100);for(var c=a[0],e=a[1],f=0;f<c.length;f++){var g=$("#"+this.id).highcharts();g.series[0].addPoint([c[f][0],c[f][1],
e[f][1]],!1,g.series[f].data.length>=b);g.series[1].addPoint(a[2][f],!1,g.series[f].data.length>=b);g.redraw()}}});
...
p[k],q[k].tickColor=p[k]);l?$("#"+this.id).highcharts({chart:{zoomType:"x"},series:r,xAxis:{type:"datetime",title:{text:h[0]},minRange:(new Date).duration(l).valueOf()},yAxis:q}):$("#"+this.id).highcharts({series:r,xAxis:{type:"datetime",title:{text:h[0]}},yAxis:q})},point:function(d,a){var b=$(this).attr("data-count");1>b&&(b=100);for(var c=0;c<a.length;c++)if(a[c]){for(var e=$("#"+this.id).highcharts(),f=0;f<a[c].length;f++)e.series[c].addPoint(a[c][f],!1,e.series[c].data.length>=b);e.redraw()}}});
...
minRange:(new Date).duration(g).valueOf()},yAxis:{min:$(this).attr("data-ymin"),max:$(this).attr("data-ymax"),title:{text:f[1]}}}):$("#"+this.id).highcharts({series:h,xAxis:{type:"datetime",title:{text:f[0]}},yAxis:{min:$(this).attr("data-ymin"),max:$(this).attr("data-ymax"),title:{text:f[1]}}})},point:function(d,a){var b=$(this).attr("data-count");1>b&&(b=100);for(var c=0;c<a.length;c++)if(a[c]){for(var e=$("#"+this.id).highcharts(),f=0;f<a[c].length;f++)e.series[c].addPoint(a[c][f],!1,e.series[c].data.length>=
b);e.redraw()}}});
...
center:["95%","90%"],size:35,showInLegend:!1,dataLabels:{enabled:!1}}],xAxis:{type:"datetime"},yAxis:{min:$(this).attr("data-min"),max:$(this).attr("data-max"),title:{text:c[1]}},tooltip:{formatter:function(){return this.series.name+" <b>"+(this.percentage?this.y.transUnit("%"):this.y.transUnit("temp"))+"</b>"}}})},point:function(d,a){var b=$(this).attr("data-count");1>b&&(b=100);for(var c=0;c<a.length;c++){var e=$("#"+this.id).highcharts();if(a[c]&&(0==c||1==c))for(var f=0;f<a[c].length;f++)e.series[c].addPoint(a[c][f],
!1,e.series[c].data.length>=b);e.redraw()}}});
